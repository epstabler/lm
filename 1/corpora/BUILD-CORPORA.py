...
(those are: Mandarin, English, JaneAutensEnglishNovel-Emma, French, Hindi, Polish, Russian, Spanish, Thai)
...
( It is easy to extend this script to use additional UD corpora.
These languages chosen because also covered fairly well by epitran. )

On my laptop, building all corpora, this script takes some time to run!
"""
...
def ud(lang):
    if lang == 'cmn':    
        ...
        return 'ud-treebanks-v2.12/UD_English-EWT/en_ewt-ud-train.conllu'
    elif lang == 'fra':
        ...
        raise ValueError('ud lang name error')

def ep(lang):
    if lang == 'cmn':
        return epitran.Epitran('cmn-Hant', cedict_file="/Users/es/.local/src/epitran/cedict_1_0_ts_utf-8_mdbg.txt")
    elif lang == 'emma':
...
def udName(lang):
    if lang == 'emma':
        ...
        pathParts = ud(lang).split('/')
        udf = pathParts[-1].split('-')
        udfParts = udf[0].split('_')
        if len(udfParts) > 1:
            return '-'.join([lang,udfParts[1]])
        else:
            raise ValueError('UD corpus name format error: %s' % lang)

VERBOSE = False

def adjustTxt(lang, txt):
    """ text adjustments, specialized to each corpus """
    if lang == 'cmn':
        ...
        # fix some minor (rare) things that might appear in examples
...
def adjustIPA(ipa):
    ...
    # adjust epitran representations of common English diphthongs
...
def buildCorpus(langs):
    ...
    for lang in langs:
        ...
        if lang == 'emma':
            ...
            corpusListed = pyconll.load_from_file(ud(lang))
...
if __name__ == '__main__':
    if not len(sys.argv) > 1:
        ...
        buildCorpus(['cmn', 'emma', 'eng', 'fra', 'hin', 'pol', 'rus', 'spa', 'tha'])
    else:
